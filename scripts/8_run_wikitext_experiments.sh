#!/bin/bash
#
# Run WikiText experiments for 3 different model sizes
...
echo "=========================================="
echo "WikiText Pretraining Experiments"
echo "=========================================="
...
echo "3. 4B model (batch_size=2, grad_accum=4)"
echo ""
...
echo "----------------------------------------"
$SCRIPT_DIR/phase2b_rtx4090.sh --model 0.6b --experiments 8 \
...
echo "[2/3] Starting 1.7B model experiment..."
echo "Time: $(date +"%Y-%m-%d %H:%M:%S")"
echo "----------------------------------------"
$SCRIPT_DIR/phase2b_rtx4090.sh --model 1.7b --experiments 8 \
    --batch-size 4 --eval-steps 1000 --eval-batches 100 \
    --save-steps 1000 --save-total-limit 2 --gradient-accum 2

if [ $? -eq 0 ]; then
    echo "[2/3] 1.7B model experiment completed successfully!"
else
    echo "[2/3] ERROR: 1.7B model experiment failed!"
    exit 1
fi
...
# Experiment 3: 4B model
echo "[3/3] Starting 4B model experiment..."
echo "Time: $(date +"%Y-%m-%d %H:%M:%S")"
echo "----------------------------------------"
$SCRIPT_DIR/phase2b_rtx4090.sh --model 4b --experiments 8 \
    --batch-size 2 --eval-steps 1000 --eval-batches 100 \
    --save-steps 1000 --save-total-limit 2 --gradient-accum 4

if [ $? -eq 0 ]; then
    echo "[3/3] 4B model experiment completed successfully!"
else
    echo "[3/3] ERROR: 4B model experiment failed!"
    exit 1
fi
...
echo ""
echo "Summary:"
echo "  ✓ 0.6B model trained on WikiText"
echo "  ✓ 1.7B model trained on WikiText"
echo "  ✓ 4B model trained on WikiText"
echo ""
echo "Check the following directories for results:"
echo "  - runs/*_qwen3_0.6b/wikitext/"
