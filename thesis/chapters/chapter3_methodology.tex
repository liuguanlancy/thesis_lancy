...
We evaluate \textbf{10 pretraining configurations}: two mixtures (Financial; Wiki+Financial) and eight single-dataset baselines. Each configuration trains at three sizes (0.6B/1.7B/4B) with a fixed \textbf{100M-token budget}. We evaluate on \textbf{8 held-out test sets} and run six follow-ups with smaller learning rates to address stability at larger scale. \Cref{tab:exp_settings} summarizes the settings: fix tokens, vary size and data.
...
Why Qwen3? Three reasons. Consistent architecture across sizes enables clean comparisons. Strong baselines on general and domain tasks. Inference is efficient—these models fit on consumer hardware for edge deployment. In practice, this makes experimentation faster.
