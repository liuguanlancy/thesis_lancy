...
We showed that \textbf{diverse in-domain mixtures outperform general-domain pretraining} for financial applications. Mixed Financial (7 datasets, 322M tokens total with a 50\% cap) achieved 21.55 ppl at 4B with 55\% CV across financial tasks. WikiText reached 48.7 ppl mean with 78\% CV (100M tokens). Adding WikiText (Mixed Wiki+Financial) degraded financial performance by 24\% (26.69 ppl) while improving general-domain by only 16\%. For finance, that trade-off is poor. \Cref{fig:scaling_comparison_all} shows the gap widening with size; cross-dataset tables show Mixed Financial rows often in boldface.
