...
\textbf{News Articles (Lettria, 197M tokens)}: Training ran 2–3 epochs across sizes with minimal overtraining. Performance on the News test set improves cleanly with scale (0.6B: 52.25 ppl; 1.7B: 22.91; 4B: 17.47), i.e., 56\% from 0.6B→1.7B and a further 24\% from 1.7B→4B. Transfer is strongest to SEC (33.46 ppl) and Alpaca (29.75 ppl), moderate to FiQA (31.69 ppl) and FinGPT (38.03 ppl), and weak on Twitter (38.98 ppl) and Financial QA (38.90 ppl). The 4B model shows 65.53\% relative spread, among the lowest for individual datasets, so generalization is reasonably consistent.

\textbf{SEC Reports (80M tokens)}: Training ran up to 24 epochs depending on size (moderate overtraining). On the SEC test set, scaling behaves as expected (0.6B: 41.12 ppl; 1.7B: 19.36; 4B: 15.91). Transfer is strong to News (16.67 ppl, similar long‑form structure), moderate to FinGPT (18.68) and Alpaca (18.54), and weaker on short‑form tasks (FiQA 19.34, Twitter 18.12, Financial QA 17.39). The 4B SEC model shows 19.32\% relative spread across evaluations.
...
\textbf{FinGPT Sentiment (19M tokens)}: Training ran for 30 epochs (the smallest model shows overtraining). On its own test set, performance scales strongly (0.6B: 32.78 ppl; 1.7B: 9.56; 4B: 5.67). Transfer aligns with instruction format: strong to Alpaca (8.27) and FiQA (8.16), weaker to document datasets (News 7.92; SEC 6.20). The 4B model’s relative spread is 37.07\%, reflecting task‑type specialization.

\textbf{Finance Alpaca (17M tokens)}: Training ran 12 epochs (moderate overtraining). On Alpaca, scaling is clear (0.6B: 63.73 ppl; 1.7B: 15.61; 4B: 8.22). Transfer is best to FiQA (9.22) and FinGPT (9.18), and poor to documents (News 8.58; SEC 8.25) and Twitter (8.97). The 4B model’s variance (11.51\% spread) reflects its narrow task focus.

\textbf{FiQA (4M tokens)}: We trained 7 epochs (short examples; near the overtraining threshold). On FiQA, scaling is strong (0.6B: 64.75 ppl; 1.7B: 12.99; 4B: 7.08). Transfer fits conversational Q\&A: good on Alpaca (7.12) and FinGPT (7.01), poor on long‑form (News 7.43; SEC 6.14). The 4B model shows 18.97\% relative spread.
...
\textbf{Avoid}: Pure WikiText for finance (41.96 ppl mean on financial tasks), small individual datasets (<4M tokens; ~20\% spread even after LR fixes with extreme overtraining), uniform hyperparameters across sizes (invites reverse scaling), and single‑format training when you expect diverse tasks (format mismatch blocks transfer).
...
Always mix in‑domain data: even seven small‑to‑medium datasets (< 200M tokens total) outperform 100M tokens of clean general text on domain tasks. If larger models are unstable, try a smaller LR—$1\times10^{-5}$ or $5\times10^{-6}$ worked for us. Prioritize dataset diversity over raw size: seven mixed datasets (4–197M tokens) beat a single 197M dataset by 34\% on mean ppl (21.55 vs 32.82). Match formats to evaluation: long‑form models struggle on Q\&A, Q\&A models on documents, and Twitter models on almost everything else. Finally, 100M tokens is sufficient when properly mixed; avoid oversampling small datasets—50cap prevents dominance while keeping diversity.
