...
The rapid advancement of large language models (LLMs) has changed natural language processing \parencite{vaswani2017attention,radford2019language,brown2020language,touvron2023llama}. But applying them in finance still faces practical hurdles. Financial institutions and individuals handle highly sensitive data—including transactions, portfolios, and trading strategies—that cannot be sent to external APIs due to privacy regulations and competitive concerns (e.g., GDPR) \parencite{eu2016gdpr}. This creates a clear need for lightweight, locally runnable financial language models that keep performance while protecting data.

In practice, domain adaptation follows two paths: training huge models from scratch or fine-tuning general models on domain data. The first path is too expensive for most; the second often misses domain nuances \parencite{gururangan2020don}. Also, the common assumption that high-quality general corpora (e.g., Wikipedia or The Pile) always help specialized applications is not well tested \parencite{gao2020pile,raffel2020exploring,longpre2023pretrainer}.

In this thesis, we study how different data sources—both in-domain financial data and out-of-domain high-quality corpora—interact during pretraining. We focus on models in the 0.6B to 4B parameter range, which are realistic for laptops and some mobile devices while keeping acceptable performance \parencite{yang2024qwen2,xia2023sheared,team2024gemma,javaheripi2023phi}. Through systematic experiments across 10 pretraining configurations and three model sizes, we report evidence on data mixture strategies for specialized domains \parencite{wu2023bloomberggpt}.

Our investigation is timely given the rising demand for privacy-preserving AI in finance. Recent regulations such as GDPR and emerging financial data protection standards require on-device processing \parencite{eu2016gdpr}. Additionally, broader use of AI means many teams must train with limited compute. So, insights on 0.6B--4B parameter models are useful in practice.

Beyond applications, this work also adds to understanding how models learn from different data distributions. We observed one surprising pattern, sometimes called ``reverse scaling''—smaller models outperform larger ones in specific regimes. In our runs, these cases point to hyperparameter choices, not fundamental limits \parencite{kaplan2020scaling,hoffmann2022training,mccandlish2018empirical}. Put another way, tuning matters.
