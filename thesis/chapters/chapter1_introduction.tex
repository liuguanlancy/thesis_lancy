...
\textbf{Chapter 4: Results} groups findings by theme and backs them with plots and tables (11 scaling figures, 18 tables). We start from data mixture effects (the core finding), then look at single datasets (component effects), training dynamics and LR sensitivity, and finally domain transfer. The scaling figures track performance across sizes; the cross-dataset tables make it easy to see which training setup wins where.

\textbf{Chapter 5: Discussion} ties the results back to prior work and practice. We comment on why WikiText underperforms on finance (see the boldface patterns in the cross-dataset tables), what in domain diversity buys us (read from the scaling plots), how LR tweaks changed stability (match dashed vs solid lines), and we finish with concrete guidelines that point to the exact tables and figures.

\textbf{Chapter 6: Conclusion} sums up contributions, notes implications, and lists near term next steps: try larger models, test dynamic mixing, and check downstream tasks.

\section{Scope and Limitations}

This thesis focuses on pretraining dynamics for causal LMs in the 0.6B to 4B range on financial text. A few scope limits matter:

\textbf{Model Architecture:} All runs use Qwen3. The learning rate/mixture takeaways likely carry over, but checking LLaMA, Gemma, Phi would help.
