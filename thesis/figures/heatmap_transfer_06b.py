...
Builds the 0.6B heatmap directly from experimental_results markdown tables.
"""
...
def main() -> None:
    train_labels, eval_labels, data = build_rows_for_size("06b")
    ...
    sns.heatmap(
        log_data,
        mask=mask,
        annot=np.where(mask, np.nan, data),
        fmt=".1f",
        cmap="RdYlGn_r",
        vmin=0.6,
        vmax=2.0,
    ...
    ax.set_title(
        "Cross-Dataset Transfer at 0.6B Model Size\n(Lower perplexity = better transfer)",
        fontsize=13,
        fontweight="bold",
        pad=20,
    )
    ...
    output_path = os.path.join(os.path.dirname(__file__), "heatmap_transfer_06b.png")
